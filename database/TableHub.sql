...
/*
    ...
    ESTRUTURA DO BANCO (vers�o compat�vel sem THROW)

    IMPORTANTE:
    1) Execute primeiro o arquivo 01_Criar_Banco_TableHub.sql.
    2) Depois execute este arquivo.
    3) Esta vers�o usa RAISERROR em vez de THROW para funcionar em
       ambientes/targets SQL Server mais antigos e evitar os erros
       de IntelliSense mostrados no editor.
*/

/* Abra esta consulta j� conectado ao banco TableHub.
   Esta verifica��o evita criar objetos acidentalmente em master. */
...
/* ============================================================
   RN010 - Perfis s�o entidades pr�prias, n�o texto livre.
   ============================================================ */
...
VALUES
    (N'Administrador', N'Gerencia usu�rios, produtos, categorias e configura��es.'),
    (N'Cozinha',       N'Visualiza e atualiza o fluxo de preparo dos pedidos.'),
    (N'Gar�om',        N'Acompanha pedidos, mesas e entregas.');
GO

/* ============================================================
   RN012 - Usu�rios guardam senha somente como hash.
...
/* ON DELETE NO ACTION em Perfis: impede apagar um perfil ainda usado por usu�rio (RN010). */
...
/* ============================================================
   ...
   Exclus�o l�gica via ativo evita quebrar produtos hist�ricos.
   ============================================================ */
...
/* ============================================================
   ...
   RN008 - Auditoria m�nima: alterado_por e alterado_em.
   Regra monet�ria - pre�o sempre DECIMAL(10,2), nunca FLOAT/REAL.
   ============================================================ */
...
/*
   ON DELETE NO ACTION em Categoria: categoria utilizada n�o pode sumir e quebrar produto.
   ON DELETE NO ACTION em alterado_por: preserva a identidade do usu�rio da auditoria (RN008).
*/
...
/* ============================================================
   ...
   estoque = 0 ou ativo = 0 => indispon�vel.
   estoque > 0 e ativo = 1 => dispon�vel.
   ============================================================ */
...
BEGIN
    ...
    /* Evita recurs�o causada pelo UPDATE executado pelo pr�prio trigger. */
...
/*
   RN008 + preserva��o de hist�rico:
   DELETE de produto vira exclus�o l�gica, mantendo refer�ncias de pedidos antigos.
*/
...
/* ============================================================
   RN005 - Rela��es de transi��o v�lidas garantidas pelo banco.
   ============================================================ */
...
/* ON DELETE NO ACTION: status utilizado pelo fluxo n�o pode ser removido. */
...
/* ============================================================
   ...
   RN004: criado_em � gerado pelo SQL Server com GETDATE().
   RN009: excluido_em implementa soft delete.
   confirmado permite criar pedido + itens atomicamente antes de congel�-lo.
   ============================================================ */
...
/*
   ON DELETE NO ACTION em Mesa, Status e Usu�rio preserva o hist�rico do pedido (RN009).
   �ndices suportam fila por chegada e filtro por status (RN004/RN005).
*/
...
/* ============================================================
   RN003/RNF005 - Itens s�o separados do cabe�alho do pedido.
   preco_unitario � snapshot: altera��o futura do pre�o do produto
   n�o modifica pedidos antigos.
   ============================================================ */
...
/* ON DELETE NO ACTION em Pedido/Produto preserva hist�rico (RN003/RN009). */
...
/* ============================================================
   RN003 - Depois da confirma��o, itens ficam imut�veis no banco.
   A regra vale para INSERT, UPDATE e DELETE, evitando altera��es diretas.
   ============================================================ */
...
BEGIN
    ...
    BEGIN
        RAISERROR(N'RN003: itens de pedido confirmado s�o imut�veis.', 16, 1);
...
/*
   Mant�m valor_total consistente durante a montagem de pedidos ainda n�o confirmados.
   Se a opera��o for inv�lida em pedido confirmado, o trigger RN003 faz rollback da instru��o inteira.
*/
...
/* ============================================================
   RN005 - Hist�rico das mudan�as de status.
   ============================================================ */
...
/* ON DELETE NO ACTION preserva a trilha hist�rica de status (RN005/RN009). */
...
/* ============================================================
   ...
   2) mudan�as respeitam Transicoes_Status_Pedido;
   3) cada status � registrado no hist�rico.
   ============================================================ */
...
BEGIN
    ...
    /* Atualiza��o de status precisa existir na tabela de transi��es v�lidas. */
    IF EXISTS (
        SELECT 1
        FROM inserted i
    ...
    BEGIN
        RAISERROR(N'RN005: transi��o de status inv�lida.', 16, 1);
        ROLLBACK TRANSACTION;
        RETURN;
    END;

    /* Hist�rico do status inicial. */
    ...
    /* Hist�rico de cada mudan�a v�lida de status. */
    INSERT INTO dbo.Historico_Status_Pedido (pedido_id, status_id, alterado_por, alterado_em)
...
/* ============================================================
   RN009 - DELETE f�sico de pedido � proibido.
   Um DELETE solicitado pela aplica��o vira soft delete.
   ============================================================ */
...
/* ============================================================
   Integra��o com o PaymentService existente no backend.
   Status financeiro fica separado do status operacional RN005.
   Assim PAGO/CANCELADO n�o corrompe o fluxo da cozinha.
   ============================================================ */
...
/* ON DELETE NO ACTION em Pagamentos preserva hist�rico financeiro do pedido. */
...
/* ============================================================
   ...
   PRIMARY KEY impede o mesmo produto duplicado na mesma requisi��o.
   ============================================================ */
...
/* ============================================================
   ...
   Cria cabe�alho, itens, baixa estoque, calcula total e confirma o
   pedido dentro de UMA transa��o at�mica.
   UPDLOCK/HOLDLOCK evita duas vendas simult�neas do mesmo estoque.
   ============================================================ */
...
BEGIN
    ...
    BEGIN TRY
        ...
        /* Bloqueia as linhas dos produtos durante valida��o para evitar overselling. */
        IF EXISTS (
            SELECT 1
            FROM @itens i
        ...
        BEGIN
            RAISERROR(N'Existe produto inexistente, indispon�vel ou sem estoque suficiente.', 16, 1);
        END;
        ...
        BEGIN
            RAISERROR(N'Status inicial Recebido n�o est� configurado.', 16, 1);
        END;

        /* Pedido nasce n�o confirmado apenas durante esta transa��o. */
        ...
        /* Snapshot do pre�o atual do produto no momento da compra. */
        ...
        /* Baixa de estoque na mesma transa��o. */
        UPDATE p
...
/* ============================================================
   RN005 - Procedure segura para avan�ar status.
...
BEGIN
    ...
    BEGIN
        RAISERROR(N'Status informado n�o existe.', 16, 1);
        RETURN;
    END;
    ...
    IF @@ROWCOUNT = 0
    BEGIN
        RAISERROR(N'Pedido n�o encontrado ou removido logicamente.', 16, 1);
        RETURN;
    END;
END;
GO
...
/* ============================================================
   Views �teis para o backend e dashboards.
...
/* ============================================================
   DADOS B�SICOS OPCIONAIS PARA PRIMEIROS TESTES
   (sem criar usu�rio/senha fict�cios).
   ============================================================ */
INSERT INTO dbo.Categorias (nome, descricao)
VALUES
    (N'Pratos', N'Pratos principais'),
    (N'Bebidas', N'Bebidas do card�pio'),
    (N'Sobremesas', N'Sobremesas do card�pio');
GO
